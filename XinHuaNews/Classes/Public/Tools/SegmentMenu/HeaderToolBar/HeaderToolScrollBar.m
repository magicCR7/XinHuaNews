...
//        [self addSubview:bottomLine];
        ...
        UIView *bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, Button_Size_Height, View_Size_Width, View_Size_Height - Button_Size_Height)];
        bottomView.backgroundColor = [UIColor whiteColor];
        [self addSubview:bottomView];
        self.indicator = [[UIView alloc]init];
        [self.indicator setBackgroundColor:DominantColor];
...
- (void)configWithTitleArray:(NSArray *)titleAry MaxShowNum:(NSInteger)maxNum{
    ...
    if (titleAry != nil && titleAry.count > 0) {
        ...
        for (int i = 0; i < titleAry.count; i ++) {
            ...
            [titleLable setTextColor:DominantGrayColor];
...
-(void)setIndicatorFrameWithIndex:(NSInteger)index{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        self.indicator.frame = CGRectMake(Width * index + (Width - indicator_width)/2, self.indicator.frame.origin.y, indicator_width, self.indicator.frame.size.height);
...
//    CGFloat btnWidth = View_Size_Width/self.maxShowNum;
//    float pointX = point * btnWidth;
//    CGRect frame = self.indicator.frame;
//    frame.origin.x += pointX;
////    NSLog(@"%f",frame.origin.x);
//    self.indicator.frame = frame;
//    NSLog(@"%f",self.indicator.frame.origin.x);
}
